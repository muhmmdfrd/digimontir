...
@section('content')
    ...
    <h1 class="text-2xl font-semibold mb-6">Edit Status</h1>
    <form action="{{ route('admin.statuses.update', $status) }}" method="POST" class="max-w-md space-y-4">
        @csrf
        @method('PUT')
        <div>
            <label for="code" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Code</label>
            <input type="text" name="code" id="code" value="{{ old('code', $status->code) }}" required
        ...
        <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium">Update Status</button>
    </form>
@endsection
