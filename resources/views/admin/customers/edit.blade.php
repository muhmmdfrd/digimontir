...
@section('content')
    ...
    <form action="{{ route('admin.customers.update', $customer) }}" method="POST" class="space-y-4 max-w-2xl">
        @csrf
        @method('PUT')
        <div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="latitude" id="latitude" value="{{ old('latitude', $customer->latitude) }}" readonly
            ...
            <div>
                ...
                <input type="text" name="longitude" id="longitude" value="{{ old('longitude', $customer->longitude) }}" readonly
                       class="w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-white shadow-sm bg-slate-50 dark:bg-slate-800/80">
                @error('longitude')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
        ...
        <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium">Update Customer</button>
    </form>
@endsection
...
@push('scripts')
    <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js" integrity="sha256-20nQCchB9co0qEjJ5RGg3GSlgueNugoeyRkQ2J8e2aE=" crossorigin=""></script>
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const latInput = document.getElementById('latitude');
            const lngInput = document.getElementById('longitude');
            const addressInput = document.getElementById('address');
            const defaultLat = {{ json_encode(old('latitude', $customer->latitude) ?? -6.2088) }};
            const defaultLng = {{ json_encode(old('longitude', $customer->longitude) ?? 106.8456) }};
            const hasInitial = latInput.value && lngInput.value;

            const map = L.map('map').setView([hasInitial ? parseFloat(latInput.value) : defaultLat, hasInitial ? parseFloat(lngInput.value) : defaultLng], hasInitial ? 16 : 13);
