...
@section('content')
    ...
    <form action="{{ route('admin.users.store') }}" method="POST" class="max-w-md space-y-4">
        ...
        <div>
            ...
            <select name="role_id" id="role_id" required
                ...
                @foreach ($roles as $role)
                    <option value="{{ $role->id }}" {{ old('role_id') == $role->id ? 'selected' : '' }}>{{ $role->name }} ({{ $role->code }})</option>
                @endforeach
            </select>
            @error('role_id')
                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
        ...
        <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium">Create User</button>
    </form>
@endsection
