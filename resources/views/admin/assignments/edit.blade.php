...
@section('content')
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden p-6 max-w-3xl">
        <form action="{{ route('admin.assignments.update', $assignment) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select id="customer_id" name="customer_id" class="w-full rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2" required>
                    ...
                    @foreach($customers as $customer)
                        <option value="{{ $customer->id }}" {{ (old('customer_id', $assignment->customer_id) == $customer->id) ? 'selected' : '' }}>{{ $customer->name }} ({{ $customer->address }})</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select id="technician_id" name="technician_id" class="w-full rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2" required>
                    ...
                    @foreach($technicians as $tech)
                        <option value="{{ $tech->id }}" {{ (old('technician_id', $assignment->technician_id) == $tech->id) ? 'selected' : '' }}>{{ $tech->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                <label for="status_id" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Status</label>
                <select id="status_id" name="status_id" class="w-full rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2" required>
                    ...
                    @foreach($statuses as $status)
                        <option value="{{ $status->id }}" {{ (old('status_id', $assignment->status_id) == $status->id) ? 'selected' : '' }}>{{ $status->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="date" id="scheduled_date" name="scheduled_date" value="{{ old('scheduled_date', optional($assignment->scheduled_date)->format('Y-m-d') ?? date('Y-m-d')) }}" class="w-full rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2" required>
            ...
            <div class="mb-6">
                ...
                <textarea id="description_by_admin" name="description_by_admin" rows="4" class="w-full rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-2" required>{{ old('description_by_admin', $assignment->description_by_admin) }}</textarea>
